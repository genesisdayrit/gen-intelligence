...
def find_daily_action_folder(dbx, daily_folder_path):
    """Find the folder ending with '_Daily-Action' inside the daily folder, with pagination support."""
...
def get_today_daily_action(dbx, daily_action_folder_path):
    """Fetch today's daily action file content from Dropbox."""
    system_tz = pytz.timezone(timezone_str)
    now = datetime.now(system_tz)

    # Format: "DA 2024-12-30.md"
    formatted_date = now.strftime('%Y-%m-%d')
    file_name = f"DA {formatted_date}.md"
    file_path = f"{daily_action_folder_path}/{file_name}"
    ...
    try:
        ...
        if isinstance(e.error, dropbox.files.DownloadError):
            raise FileNotFoundError(f"Daily action file not found: {file_name}")
        raise
...
def main():
    ...
    try:
        ...
        # Get today's daily action content
        content = get_today_daily_action(dbx, daily_action_folder_path)

        print("\n" + "=" * 50)
        print("TODAY'S DAILY ACTION")
        print("=" * 50 + "\n")
        print(content)
