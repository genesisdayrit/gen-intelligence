#!/usr/bin/env python3
"""Create weekly health review file in Obsidian vault via Dropbox.

Creates a numbered weekly health review file with date range (Wed-Tue)
in the vault's _Weekly/_Weekly-Health-Review folder. Auto-increments the
...
def _fetch_last_review_number(dbx: dropbox.Dropbox, health_review_folder: str) -> int:
    """Get the highest review number from existing files."""
    entries = _list_all_entries(dbx, health_review_folder)
    review_files = [
        entry.name for entry in entries
        if isinstance(entry, dropbox.files.FileMetadata)
        and entry.name.startswith("Weekly Health Review ")
    ]
    if not review_files:
        return 0
    review_numbers = []
    for f in review_files:
        try:
            review_numbers.append(int(f.split(" ")[3]))
        except (IndexError, ValueError):
            continue
    return max(review_numbers) if review_numbers else 0
...
def create_weekly_health_review_page() -> bool:
    """Create a weekly health review file in the Obsidian vault.

    Creates a numbered health review file with a Wed-Tue date range.
    ...
    try:
        ...
        last_review_number = _fetch_last_review_number(dbx, health_review_folder_path)
        ...
        if _date_range_exists(dbx, health_review_folder_path, date_range):
        ...
        file_content = (
            f"Start Date: {next_wednesday.strftime('%Y-%m-%d')}\n"
            f"End Date: {following_tuesday.strftime('%Y-%m-%d')}\n"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Create weekly health review file in Obsidian vault")
    parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(levelname)s: %(message)s")
    success = create_weekly_health_review_page()
