...
- **Rev-safe uploads on folder-journal relations.** That job captures each file's Dropbox `rev` on download and writes with `WriteMode.update(rev)` (`autorename=False`) via `services.obsidian.utils.dropbox_rev_safe`. If Obsidian or Dropbox desktop changed the file after download, the hub does **not** `WriteMode.overwrite` and does **not** create `Name (conflicted copy).md`. It logs the file as deferred (latest cloud content wins) and retries on the next 15-minute run, or once immediately after a re-download. Paths whose name already contains `conflicted copy` are skipped so the job never writes Journal YAML onto a fork. Readwise journal buffet / Knowledge Hub page writes use the same helper. Other Obsidian writers (share-link, Granola, daily properties, DA/Todoist/Manus/Telegram) still use overwrite and can adopt it later.
