...
| `reconcile_deferred_obsidian_writes` | Every 15 minutes | `services.obsidian.reconcile_deferred_writes.reconcile_deferred_obsidian_writes` | Replay deferred rev-safe writes, then light Readwise / Granola since-checks so webhook misses still land. |
...
- **Rev-safe uploads on folder-journal relations.** That job captures each file's Dropbox `rev` on download and writes with `WriteMode.update(rev)` (`autorename=False`) via `services.obsidian.utils.dropbox_rev_safe`. If Obsidian or Dropbox desktop changed the file after download, the hub does **not** `WriteMode.overwrite` and does **not** create `Name (conflicted copy).md`. It logs the file as deferred (latest cloud content wins) and retries on the next 15-minute run, or once immediately after a re-download. Paths whose name already contains `conflicted copy` are skipped so the job never writes Journal YAML onto a fork. Readwise journal buffet / Knowledge Hub page writes use the same helper. After that immediate retry still defers, the shared helper enqueues `{source, kind, payload_ref, target}` on Redis (`obsidian:deferred_writes`) so `reconcile_deferred_obsidian_writes` can replay it. Other Obsidian writers (share-link, Granola, daily properties, DA/Todoist/Manus/Telegram) still use overwrite and get queueing for free when they pass a `defer=` context into the helper.
- **`reconcile_deferred_obsidian_writes` every 15 minutes** stamps `obsidian:last_reconcile_check_at`, drains up to 50 deferred items (dead-letter after 5 attempts), then runs a Readwise export/list since-check (idempotent journal buffet / KH writers) and Granola `sync_granola_notes` (idempotent on `<!-- granola:not_… -->`; still overwrite until Granola adopts rev-safe). Manual trigger: `POST /scheduler/jobs/reconcile_deferred_obsidian_writes/run`.
