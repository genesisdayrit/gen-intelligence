...
| `reconcile_missed_obsidian_writes` | Top of every hour (`0 * * * *`) | `services.obsidian.reconcile.runner.reconcile_missed_obsidian_writes` | Batched catch-up for deferred rev-safe uploads plus Readwise / Granola since-checks. New sources plug in as providers. |
...
- **Hourly reconcile (`reconcile_missed_obsidian_writes`).** Top of every hour. Shared since-last-check cursor `obsidian_reconcile:last_reconcile_check_at` (UTC). Providers fetch `updated_at > last_reconcile_check_at` (or the equivalent API cursor). **First run only** (empty Redis) seeds `now−1h` so history is not dumped; later runs use the stored watermark as-is — 1h, 3h, or longer if the job was down. There is no today-only / same-calendar-day gate. Deferred queue items drain by `enqueued_at` / attempts, not journal date. Then since-check Readwise (export + Reader list → the same idempotent writers as the webhook) and Granola (`sync_granola_notes` with the shared watermark as `updated_after`, not Granola's 15m empty-Redis seed). Stubs sit in the registry for share-link, daily action, Todoist, Manus, and Telegram since-checks; those live paths already enqueue on defer. Spotify Music of the Day also enqueues the same journal day after a second rev mismatch. Manual: `POST /scheduler/jobs/reconcile_missed_obsidian_writes/run` with optional `?since=`. Conflict guards prevent forks; the hourly job recovers deferred and missed writes.
...
1. Subclass `BaseReconcileProvider` (or replace a `StubProvider`) in `app/services/obsidian/reconcile/providers.py`.
2. Implement `reconcile(ctx)` using `ctx.since` (`updated_at > last_reconcile_check_at`) and `ctx.batch_size`. Do not date-gate on today / calendar day.
