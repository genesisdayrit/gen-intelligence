...
- [[Note Title]]
  - [source](PUBLIC_URL)
  - [readwise](https://read.readwise.io/read/ID) · `ID`
  - Author Name
...
Standalone lines dedup on the exact wikilink bullet (`- [[Note Title]]`). Highlight lines dedup on `readwise.io/open/{id}`. The two never collapse into each other. Same-day skip does not duplicate the metadata block.
