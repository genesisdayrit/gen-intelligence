...
Readwise does not refire webhooks for historical imports. Use the repeatable `backfill_readwise_highlights` job to pull from [GET /api/v2/export/](https://readwise.io/api_deets) and append the same Content Buffet bullets. That job already calls the highlight formatter/writer, so tweet highlights also get the `Tweets from @handle` Bookmarked Tweets line — no separate backfill. Safe to rerun: existing highlight id / `readwise.io/open/{id}` bullets are skipped on both the journal and the handle page. Missing journal files are skipped (never dumped onto today). Journal writes are batched per journal day; handle-page failures do not undo those writes.
